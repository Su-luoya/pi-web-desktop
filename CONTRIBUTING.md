...
- `Scripts/scan-secrets.sh --self-test` 先用临时目录里的样本证明每条凭据规则都会命中、误报边界的样本（占位符、示例 URL、camelCase 标识符值）不会被报告、`scan-secrets: allow(reason=…)` 只跳过带标记的那一行且裸标记/空理由/过短理由不抑制并计数，再在临时 Git 仓库里证明未跟踪文件门禁（默认退出 3、加 `--include-untracked` 能扫到未跟踪文件里的样例凭据、没有未跟踪文件时行为不变；该自测因此需要 `git`）；`Scripts/scan-secrets.sh` 再扫描所有已跟踪文件，工作区没有未跟踪文件时必须退出 0，有未跟踪文件时如实退出 3（先 `git add` 或删除，本地排查时可显式加 `--include-untracked`）。提交前可以用 `./Scripts/scan-secrets.sh <file>` 只扫一个文件（未跟踪的文件也可以直接指定）。
...
- `Scripts/scan-secrets.sh`（#11 新增，CI 上由 `Self-test the secret scanner` 与 `Scan tracked files for committed secrets` 两步执行）：按固定形状扫描已跟踪文件里的 AWS access key ID（`AKIA`/`ASIA`）、GitHub token（`ghp_`/`gho_`/`ghu_`/`ghs_`/`ghr_`/`github_pat_`）、Slack token（`xox` 家族）、Stripe 与 provider key（`sk_live_`/`sk_test_`/`sk-proj-` 等）、age secret key、PEM/OpenSSH/SSH2/PGP 私钥头、JWT、`Authorization: Bearer`、带凭据的连接串，以及配置位置的键值赋值（键名大小写不敏感、允许 `AWS_SECRET_ACCESS_KEY` 这类前后缀、覆盖 JSON/YAML/TOML/`export` 形态、值至少 12 个可打印 ASCII 字符）；占位符与模板值、文档主机、camelCase 标识符值按设计整行丢弃。默认的仓库级扫描在存在未跟踪且未被 `.gitignore` 忽略的文件时**拒绝给出结论并退出 3**，消息列出前 5 条并提示 `git add <path>` 或 `--include-untracked`；该开关把未跟踪文件就地一并扫描，只用于本地排查（未 `git add` 的文件 CI 永远扫不到）。`--self-test` 除了证明规则与误报边界，还在临时 Git 仓库里断言这条门禁。
- 内联抑制：匹配行只有在**同一行**带 `scan-secrets: allow(reason=…)`（理由至少 8 个字符）时才被跳过，脚本没有按文件、目录或路径整体放行的开关；裸标记、空理由和过短理由不生效：该行照常上报、打印 `scan-secrets: warning:` 并计入结尾的 `scan-secrets: rejected N suppression marker(s)…`。每次实际执行的扫描在结尾输出 `scan-secrets: suppressed N lines`，被抑制的行仍逐行打印（`scan-secrets: suppressed: <path>:<line>:…`）；未跟踪文件导致的退出 3 在扫描前结束，不打印这两行。这个标记只允许加在确定是样例数据的行上（例如脱敏测试夹具），不允许用来消音真实或来源不明的命中。

退出码：0 表示没有命中，1 表示至少命中一处，2 表示用法/环境错误，3 表示工作区存在未跟踪且未被 `.gitignore` 忽略的文件、仓库级扫描拒绝给出结论（先 `git add`，或用 `--include-untracked` 本地排查后重跑）。被拒绝的抑制标记也算命中（该行会列在上面的命中行里），所以 `rejected N` 不为 0 时退出码是 1。
...
`scan-secrets: allow(reason=…)` 只用于样例数据，并且必须逐条评审：

- 只有“同一行”的标记生效，所以标记不会连带放过文件里的其他命中。
- 理由必须写清楚、至少 8 个字符（例如 `redaction fixture`）；裸标记与空/过短理由不抑制，会让该行上报并使扫描失败。
- 加标记的行必须在 PR 描述里说明它是样例数据以及用途；评审逐行确认该行确实不是真实凭据，而不是只看扫描器变绿。
- 把运行结尾的 `scan-secrets: suppressed N lines` 与本次 diff 仍在命中的标记数对照，并确认没有 `scan-secrets: rejected` 行；N 多于本次新增的标记数、或者标记出现在非夹具文件里，都应当先质疑再合并。
- 真实凭据、疑似凭据、来源不明的字面值一律不加标记；先按 [SECURITY.md](SECURITY.md) 处理。

## 代码和隐私要求

- 不提交构建产物、ZIP、用户路径、主机名、代理默认值、密码、token 或认证文件。
- `scan-secrets: allow(reason=…)` 内联标记只能加在确定是样例数据的行上（理由至少 8 个字符），并在 PR 里逐条说明；不要用它消音真实命中。
- 不读取、复制或迁移 Pi 的认证内容。
