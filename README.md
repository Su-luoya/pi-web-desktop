...
- 桌面应用自身的更新是一条独立路径（菜单「下载并安装桌面应用更新…」，从 `0.1.0-alpha.15` 起）：只有应用装在 `/Applications`、且本轮刚从 `api.github.com` 核实出更高版本时入口才出现，需要你确认后才下载；它只校验 Release 里的 `.zip.sha256` 与应用身份，不校验 Apple 签名链，也没有自动回滚或降级路径。`v0.1.0-alpha.14` 及更早版本没有这个入口，升级到 `0.1.0-alpha.15` 及以后需要手动下载 ZIP 并替换应用包。
