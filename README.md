...
- **什么情况下才会自动更新**（两个开关默认都关闭）：
  ...
  - **回滚能力有限**：只有在来源是已验证的 npm 全局安装、应用保留的更新前可执行文件仍在原位、仍可执行、大小与修改时间一致时，应用才会把服务（Pi Web）或后续版本重检测（Pi CLI）指回那个旧文件；其它情况一律显示「无法自动回滚」并给出可手动执行的命令（应用不执行）。更新后的验证只检查可执行文件、版本号、包名与健康检查，**不确认代码签名、不确认 Apple 公证，也不做安装包内容比对**。
...
- **更新验证有边界**：应用不做代码签名验证，也不声称确认过发布来源；「回滚」最多是把服务或版本重检测指回应用保留、且仍在原位且指纹一致的更新前 npm 全局可执行文件，不恢复文件内容、不卸载新版本。
