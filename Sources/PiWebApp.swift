...
final class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSMenuDelegate {
    ...
    /// 最近一次规划结果（诊断/设置页展示用）。
    private var piPackageUpdatePlanSet: PiPackageUpdatePlanSet?
    /// 最近一次失败的扩展包更新的持久警告（跨启动保留）。
    private var piPackageUpdateWarning: PiPackageUpdateWarning?
    /// 扩展包更新进行中。
    private var piPackageUpdateInProgress = false
    /// 菜单里的持久告警项与扩展包更新入口项。
    ...
    @objc private func showPiPackageUpdates(_ sender: Any?) {
    ...
    /// 菜单里的持久告警项：展开完整告警文本，并提供“清除警告”。
    @objc private func showPiPackageUpdateWarning(_ sender: Any?) {
    ...
    /// fixture 枚举真实进程。
    private func piPackageStatusBlockText() -> String {
        guard !appConfiguration.isSmokeLaunch else { return "" }
