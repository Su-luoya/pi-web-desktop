...
- `WebViewNavigationPolicy`（`Sources/App/WebViewNavigationPolicy.swift`）：本地/外链 URL 判定（`127.0.0.1`/`localhost`/`::1` 加配置端口；`about`/`blob`/`data` 视为内联）与导航失败判定（`WebViewNavigationFailure` 与 `navigationFailure(for:)`：取消类错误 `NSURLErrorCancelled`，含包装在 `NSUnderlyingErrorKey` 里的情形，一律不上报；其余按错误码归为 `.connectionFailed` / `.loadFailed`），不依赖 Cocoa/WebKit，可在 unhosted 测试目标里直接测试。
