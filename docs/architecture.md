...
应用启动服务时只把子进程 PID 写入 `service.pid`；应用重启后必须验证该记录对应的进程仍然存活、且命令行看起来是本项目的 `pi-web`，否则按外部服务处理。这些检查不匹配时只删除记录，不向对应 PID 发送信号。端口上存在名为 `pi-web` 的进程不足以证明应用拥有它。

更完整的进程记录（启动时间、解析后的 executable、参数摘要、实例标识）尚未实现，属于后续 issue 范围。当前命令行匹配仍是大小写不敏感的子串匹配，无法区分 `other-pi-web-helper` 这类无关进程，也无法抵抗 PID 复用；在 #5 建立可验证的所有权记录（启动时间和 executable 比对）之前，它只是弱证据。

同一原因，当用户显式选择停止或重启、而端口上的监听进程只有指令名匹配时，`ServiceManager` 目前仍可能向该监听进程发送 `TERM`；在 #5 完成所有权判定之前，应用不会在用户没有触发这些动作的情况下停止任何外部服务。
