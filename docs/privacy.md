...
Pi Web Desktop 在应用运行期间做只读的版本查询，只提示、不下载、不安装任何东西（唯一例外是下面由用户点击触发的桌面应用自身更新）。版本查询**不是项目遥测**：请求只用于比较“本机版本”与“上游最新版”，不携带使用数据、会话内容、认证信息或诊断报告。应用退出后不检查（不安装 LaunchAgent，也不在后台常驻）。另一处例外是下面两个启动前自动更新（**默认关闭**）：它们只对来源为已验证的 npm 全局（Pi CLI 还允许 pnpm 全局）安装、且目标版本来自**本次运行从白名单主机取得的网络检查结果**的组件执行一次受限更新，不改动桌面应用自身，也不涉及 Pi 扩展包——**Pi 扩展包永远不会在无人值守时更新**（见下面“扩展包更新需要用户确认”）。
...
- 桌面应用自身更新（只由用户点击触发）：菜单里的“下载并安装桌面应用更新…”只在本轮刚从 GitHub API 核实出更高版本时出现，确认框会说明当前包未经过 Apple Developer 签名与 notarization。确认后应用用独立的临时 `URLSession` 取 GitHub Release 里名字固定的资产与其 `.sha256`，哈希一致才解包并只替换 `/Applications/Pi-Web-Desktop.app`（该包必须真实存在、路径形状正确、不跳符号链接，且 `/Applications` 可写探查通过），替换后应用退出并重新启动。重定向只放行 `release-assets.githubusercontent.com` 且 zip 与 `.sha256` 不能互换，其它主机一律判失败；缓存里的检查结论不触发下载，也没有无人值守路径。
- 频率与设置：应用启动后立即检查一次。之后四类组件各自按设置复查，默认值与 [发布说明](releasing.md) 的 alpha.2 更新策略一致：桌面应用 / Pi CLI / Pi Web 默认“每日”（24 小时），可选“每周”或“关闭”；Pi 扩展包默认“检查并通知”（7 天，与 GitHub #17 的节奏相同），可选“询问后更新”或“关闭”。“询问后更新”同样只按 7 天复查，发现可用更新时询问用户是否更新（GitHub #22 起，用户在确认框里确认后才会执行一次 `pi update npm:<包名>`；未确认、取消或进程状态不确定时都不执行）；询问与确认本身不下载、不安装；真正会下载并替换文件的只有下面两个启动前自动更新（默认关闭，Pi Web 仅限已验证的 npm 全局安装，Pi CLI 仅限已验证的 npm/pnpm 全局安装且当次确认没有运行中的 Pi 进程）与上面那处用户点击触发的桌面应用自身更新。
...
- 启动前自动更新 Pi CLI 与运行进程保护（GitHub #21，默认关闭）：打开后，应用在拿到当次版本检查结果后（只在本机 Pi CLI 的来源为**已验证的 npm/pnpm 全局安装**、目标版本已验证且高于本机版本、**判定所用的检查结果来源是本次运行从白名单主机取得的网络响应**（GitHub #59：缓存回退不参与自动更新）、并且当次进程检查确认**没有运行中的 Pi CLI** 时）执行一次 `pi update --self`。
  - **如何判断“有运行中的 Pi CLI”**：应用只读地枚举本机进程（`proc_listpids` 枚举 PID；`proc_pidinfo` / `proc_pidpath` 取父进程 PID、启动时间、内核进程名与真实镜像路径；脚本与进程标题靠 `sysctl KERN_PROCARGS2` 得到的 argv 判定），**不看、不写其它进程的任何内容**；判定只做精确的可执行名比较（`pi-web`、`pip`、`pi-helper` 不会命中）。这是**本机只读**操作：不联网、不上传；进程检查结果不写入设置或缓存，只出现在内存、界面与（已脱敏的）日志里。读取分两步（GitHub #61）：先用上面两个便宜调用得到身份事实并判断候选进程（镜像路径或内核进程名的可执行基名恰好是 `pi`，或是 `node` / `npm` / `bun` / `deno` 这类 JS 运行时的进程），只有候选进程才继续读 argv；系统守护进程、编译器、编辑器等非候选进程不读命令行。候选判定只是读取优化，不是安全判断：镜像路径与进程名都读不到、权限不足或枚举失败仍然按“不确定”处理。脚本路径必须既是**绝对路径**、又能解析出可执行文件（可执行位，或符号链接能解析到存在的目标）才算命中；相对路径（`./pi`、`../bin/pi`，或解释器脚本位置的裸 `pi`）与断裂的符号链接一律按“不确定”处理，既不会当成“不是 Pi”（漏判方向），也不会被当成已确认的 Pi。只要有任何 Pi CLI 在运行、或者枚举/读取失败因而无法确认，应用就**不自动更新**（推迟到下次启动或下一次判定），并把原因写入日志与状态页。
  - **命令摘要的遮罩与已知边界**：进程记录里的命令摘要先按 token 边界遮罩，再经 `LogRedactor` 整体脱敏，最后截断到 200 字符。遮罩覆盖：`--token=<值>` 这类键值形态（保留键；键名大小写不敏感，`--PASSWORD=<值>` 同样命中），长开关 `--password` / `--token` / `--api-key` 的裸形态（值在下一个 token），短开关 `-p` / `-t` / `-s`（大小写不敏感，`-P` / `-T` / `-S` 同）与多字母简写 `-pw` 的三种取值形态——`-p<值>`、`-p=<值>`、`-p <值>`；裸开关的取值在下一个 token 时，下一个参数不以 `-` 开头就只遮那一个 token（后续参数仍可读），以 `-` 开头则无法可靠区分“值”与“下一个开关”，连尾巴一起隐藏（宁可少展示，不少脱敏）；纯短开关字母组成的开关组（`-pt`）按裸开关同样处理。此外覆盖 `sk-` / `ghp_` / `xoxb-` 这类已知凭据前缀、长度 ≥ 32 的 base64/十六进制形状的位置参数，以及 URL 查询串（`?token=<值>` 整段换占位符）；`KEY=VALUE` 环境片段直接丢弃，Home 路径换成 `~`。**已知边界（如实说明，不夸大）**：这是模式化遮罩，不是“凡秘密必被遮”——不符合上述形状的自由文本会**原样保留**，例如短于 32 字符、又没有已知前缀或敏感键名的位置参数（口令、会话串），以及与键名无关的普通句子；反过来，`-p` 无法区分“密码”与“端口”这类同名开关，因此 `-p 8080` 这类非秘密数值也会被遮成 `-p <占位符>`，`-port` 这类以敏感字母开头的开关会显示成 `-p<占位符>`。因此不要把秘密直接放进命令行；如果必须传，优先用环境变量或配置文件，并且不要依赖本应用的摘要脱敏。这个保留行为与上述近似样例取舍在 `PiWebDesktopTests/PiProcessInspectorTests.swift` 里有明确断言（不是遗漏）。
  ...
  - **只读磁盘的部分**：判断“名为 `pi` 的脚本路径”时会读可执行位（元数据）、是否为符号链接及其解析后的真实路径（路径解析与存在性，都只读元数据），不读文件内容、不写任何文件。
...
公开 Issue 或 PR 不得包含密码、token、API key、代理凭据、查询参数、用户名、Home 路径、私人主机名或完整私有日志。日志行、错误消息、环境变量与命令行展示、诊断导出共用 `Sources/Diagnostics/LogRedactor.swift` 里的同一个实例，至少覆盖：URL 查询串整体替换、`Authorization:` 头、`Bearer <token>`、`token=`/`password=`/`secret=`/`api_key=`/`apikey=` 形式的键值（含 JSON 与 `PI_WEB_PASSWORD`）、JWT 形态字符串、代理凭据（`scheme://user:pass@host`）、Home 路径（包含非当前用户的以 `/Users` 开头的路径，替换为 `~`）、私钥头与私钥块；多行输入逐行处理。菜单“复制诊断”与诊断窗口的复制按钮在写入剪贴板前会弹出脱敏提醒，说明文本已按规则脱敏、但公开粘贴前仍需自行检查。完整规则表与诊断导出字段见 [日志与诊断导出](logging-and-diagnostics.md)。

脱敏不替代用户提交前的自查。

## 登录 shell PATH 查询（GitHub #89）

应用从 Finder / Dock 启动时进程 `PATH` 只有系统目录，`node` 可能只在用户 shell 的 `PATH` 里。为了在不要求用户手工配环境变量的前提下找到 `node`/`pi`/`pi-web`，应用会执行一次**用户自己的登录 shell**，只读取它报告的 `PATH` 值：

- 命令：`<登录 shell> -lc 'printf '__PI_WEB_TOOL_PATH__%s' "$PATH"'`（拿不到值时再尝试一次 `-ilc`）；登录 shell 优先取用户数据库里的值（与 `dscl . -read ~ UserShell` 同源），再取 `$SHELL`，最后退回 `/bin/zsh`。
- 频率：全进程各一次，带 3 秒上限；失败、超时、空输出都当作“没有这个值”，继续用应用 `PATH` 与已知目录兜底，不会阻塞诊断页。
- 本机只读：不联网、不写任何文件、不修改 shell 配置、不安装任何东西；子进程 `stdin` 固定为 `/dev/null`，`stderr` 丢弃。

需要如实说明的两点：

1. 登录 shell 会像用户打开终端一样加载**用户自己的** shell 配置文件（`~/.zshenv`、`~/.zprofile`、`~/.zshrc` 等）；这是登录 shell 的固有行为。应用自己不读取、不解析、不复制这些文件的内容，也不把它们写入日志或诊断报告，只使用命令输出里标记之后的 `PATH` 值。交互式兜底（`-ilc`）同理，因此用户 rc 里的副作用仍属于用户自己的配置。
2. 查询结果只用于在内存里构造子进程的 `PATH`，不落盘；诊断导出里的环境变量展示仍经过 `LogRedactor` 脱敏，凭据类键不会进入子进程环境。

如果用户把 `PATH` 配在了应用无法自动覆盖的地方（例如只在某个屏幕会话或 `launchctl` 会话里），诊断条目会给出可读原因而不是只说“无法确定”，具体的手动兜底做法见 [日志与诊断导出](logging-and-diagnostics.md#手动兜底faq)。
