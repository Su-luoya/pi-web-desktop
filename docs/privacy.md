...
- 缓存不是可信输入（GitHub #59）：缓存文件就在用户的 Application Support 下，同一用户（或任何能以该用户身份写文件的程序）可以改写它，应用不做签名校验，因此**缓存内容不参与自动安装判定**。读取时应用会先看文件大小上限，再逐项校验结构与形状——JSON 与字段类型可解码、schema 版本一致、条目数不超上限、分类/包名/目标 id 互相一致、枚举值已知、版本字符串是规范化的语义化版本、时间戳不落在未来、条件请求字段长度受限。任何一项不合法或损坏，应用就丢弃**整份**缓存（不崩溃、不部分采用），写一条固定原因的日志（不回显缓存内容或路径），并把该分类当作“没有缓存”处理：网络失败时仍可在界面与诊断里提示“上次检查结果显示有更新”，但会同时标注来源为**本机缓存**与缓存写入时间，且不写“已验证/官方”之类会让人以为本次已由上游确认的措辞。检查结果的来源因此分三档：本次网络响应（`network`）、缓存回退（`cached-fallback`）、没有可用结果（`unavailable`）；条件请求命中 304 也属于缓存回退（上游只确认了“缓存里的那个版本仍是最新”，版本值本身来自本机文件）。只有 `network` 允许驱动自动更新。
- 可信度与来源：只有响应来自预期域名且结构可解析时才将上游版本标为“已验证”；网络失败、超时、限流（HTTP 429）与 5xx 沿用 24 小时/7 天内上一次成功结果并标注为缓存结果（同时标注来源与缓存写入时间）；超过有效期、响应无法解析或来自非预期主机时显示“无法确定”。来源与可信度是两件事：`cached-fallback` 可以带着上次的“已验证”可信度用于提示，但**不会**被当成自动安装的依据（GitHub #59）。检查失败只影响提示文案，不影响正在运行的服务，也不改变服务状态。
