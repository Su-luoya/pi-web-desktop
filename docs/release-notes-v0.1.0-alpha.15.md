...
| 从旧版本升级 | `v0.1.0-alpha.14` 及更早版本没有自更新入口，升级到本版需要手动下载 ZIP 并替换应用包；应用内更新从本版（`0.1.0-alpha.15`）开始生效 |
...
- 更新后的应用能直接启动，不需要再手动放行一次 Gatekeeper（手动下载的 ZIP 仍然需要按「安装」一节
  手动放行）。
- 上面这一轮端到端验证用的是 `v0.1.0-alpha.14` 的真实发布资产，发生在 PR #178 修好两处缺陷之后。
- **本版资产（`v0.1.0-alpha.15`）发布后没有再跑一轮真机端到端复验**：维护者在发布后决定跳过这轮
  （时间与理由见 [Release Issue #179](https://github.com/Su-luoya/pi-web-desktop/issues/179) 的发布后评论）。
  所以本版保留的是 alpha.14 周期这份验证记录，加上发布后在公开资产上做的独立复核（校验值 / 版本 /
  签名，见「校验值」与 `docs/alpha-release-checklist.md` 的对应记录）。
...
发布资产由 `.github/workflows/release.yml` 在 tag `v0.1.0-alpha.15` 上生成；**本节的数值已按发布后的公开资产回填**
（不用本机演练值冒充发布值）。
...
| SHA-256 | `7721d2403d8335f4439ada9b92adeba13a890480dd79bbeaaedc9126ccfe1c8a`（发布后实测；演练值 `f62359c7…9eab`） |
...
| 签名与 Gatekeeper | `codesign --verify --deep --strict` 退出 0；`spctl -a -vv` 退出 3（ad-hoc 未公证的预期结果） |
...
- **换成另一个 ad-hoc 构建后首次启动可能要求钥匙串授权（本版新增）**：把 `/Applications` 里的应用
  换成另一个 ad-hoc 构建（包括用应用内更新装上的新版本）后，macOS 可能弹窗要求授权新的可执行文件
  读取已保存的远程访问密码。这是 ad-hoc 签名的 CDHash 变化导致的预期行为，不是更新失败；授权或
  拒绝都不影响应用启动、服务与其它功能。
...
- **真机 GUI 未全部手工验收**：本机只有 Command Line Tools、没有 Xcode，本版门槛是脚本（构建 / 身份 /
  ...
  版本」），**本版资产发布后的那一轮复验没有做**（维护者选择跳过，见 #179 的发布后评论）；其余 GUI
  行为（窗口、菜单、诊断页）的运行时行为仍以各版的真机验证表为准。
...
| 项 | 步骤 | 期望 | 状态 |
| --- | --- | --- | --- |
| 自更新入口出现 | 用低于本版的构建装进 `/Applications`，点「服务 → 更新检查设置 → 检查更新…」 | 菜单出现「下载并安装桌面应用更新…」 | 已在 alpha.14 周期验证（见「本机实测环境与版本」）；本版资产发布后的复验未执行（维护者选择跳过） |
| 确认与安装 | 点该菜单项 → 确认框写明目标版本 → 点「更新」 | 下载、校验、替换 `/Applications` 应用，应用退出并重启 | 已在 alpha.14 周期验证；本版资产发布后的复验未执行（维护者选择跳过） |
| 安装结果可核对 | 更新后核对 `/Applications/Pi-Web-Desktop.app` | 版本为 `0.1.0-alpha.15`，可执行文件 SHA-256 与 Release 资产一致 | 本版未执行（维护者选择跳过真机复验） |
| 校验失败即中止 | 把下载到的 `.sha256` 换成不匹配的值后再触发更新 | 报「更新包校验失败，已停止安装」，`/Applications` 应用不变 | 本版未执行（维护者选择跳过真机复验） |
| 非 `/Applications` 拒绝 | 把同一个构建放到 `/tmp` 运行后点该菜单项 | 报「当前应用不是从 /Applications 安装，无法自动更新。」 | 本版未执行（维护者选择跳过真机复验） |

本版没有完成上面这轮复验；这 5 项仍然是后续版本发布前的验收项，跳过的决定与时间见
[Release Issue #179](https://github.com/Su-luoya/pi-web-desktop/issues/179) 的发布后评论。
