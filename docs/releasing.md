...
- `alpha.3`：受限自动安装（GitHub #20：仅限“已验证的 npm 全局安装”的 Pi Web，启动前自动更新 + 安装后版本重检测与健康检查）、运行进程保护（GitHub #21：只读进程枚举三态判定 + 有 Pi 进程运行时自动更新延迟 + Pi CLI 的受限启动前自动更新与手动入口）、更新验证与有限回滚（GitHub #22 / #23）。当前分支已落地 #20 与 #21：失败时保留旧版本语义、进入诊断页并写持久警告，**不声称回滚**；#21 不做任何进程处理（不发送信号、不结束 Pi 会话），自动更新只使用官方 `update --self` 参数数组。下载缓存、更细的回滚策略与非 npm/pnpm 来源的自动更新仍属后续 issue。
...
`v0.1.0` 不实现应用自身（`Pi Web Desktop.app`）的应用内更新：应用只在后续版本检查 GitHub Release 并提示版本，不下载、不安装桌面应用本身。检查行为由设置控制：四类组件默认“每日 / 扩展包检查并通知”，可逐类关闭或改为每周，可忽略某个具体版本（上游出现更高版本会重新提示）；提示使用应用内提示框，不使用系统通知中心。设置里的“启动前自动更新 Pi Web”开关自 alpha.3 实现（GitHub #20）起生效，但只对“来源为已验证的 npm 全局安装”的 Pi Web 执行受限安装；同一版本新增的“启动前自动更新 Pi CLI”（GitHub #21）也只对“来源为已验证的 npm/pnpm 全局安装”的 Pi CLI 生效，并且在检测到运行中的 Pi 进程或进程状态不确定时自动推迟（应用不会向 Pi 进程发送任何信号）；其它来源仍只显示命令，桌面应用自身仍不自动更新。
