...
- `alpha.3`：受限自动安装（GitHub #20：仅限“已验证的 npm 全局安装”的 Pi Web，启动前自动更新 + 安装后版本重检测与健康检查）、运行进程保护（GitHub #21）、更新验证与有限回滚（GitHub #22 / #23）。当前分支只落地 #20：失败时保留旧版本语义、进入诊断页并写持久警告，**不声称回滚**；运行中的服务保护与非 npm 来源的自动更新仍属后续 issue。
...
- 更新前记录当前版本。只有安装来源和工具提供可靠恢复路径时才执行回滚；不能承诺所有 npm、
  ...
  并把完整但已脱敏的日志留给用户查看。GitHub #20 的实现遵循这一条：失败只保留旧版本
  语义（不卸载、不重装、不自动回滚），并写一条持久警告与诊断页提示。
- 如果 tag 推送后需要撤回：先删除草稿 Release，再删除 tag
  （`git push origin :refs/tags/<tag>`），并在 Release Issue 记录原因；不要重复使用已公开的版本号。

## 应用自身更新

`v0.1.0` 不实现应用自身（`Pi Web Desktop.app`）的应用内更新：应用只在后续版本检查 GitHub Release 并提示版本，不下载、不安装桌面应用本身。检查行为由设置控制：四类组件默认“每日 / 扩展包检查并通知”，可逐类关闭或改为每周，可忽略某个具体版本（上游出现更高版本会重新提示）；提示使用应用内提示框，不使用系统通知中心。设置里的“启动前自动更新 Pi Web”开关自 alpha.3 实现（GitHub #20）起生效，但只对“来源为已验证的 npm 全局安装”的 Pi Web 执行受限安装；其它来源仍只显示命令，桌面应用自身仍不自动更新。
