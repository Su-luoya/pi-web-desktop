...
- 界面：菜单“服务 → 更新检查设置”里是状态行 + 四类快捷开关（打开 = 默认策略，关闭 = 关闭）+ “立即更新 Pi Web…”与“立即更新 Pi CLI…” + 失败告警条目 + “更新检查偏好设置…”；偏好设置窗口显示策略、每类最近检查时间、结果（最新 / 可更新 / 未知 / 失败）、被忽略版本与下次检查时间，并提供“忽略此版本”、两个启动前自动更新开关（标题写明生效范围）与一行 Pi CLI 进程保护/推迟原因状态（GitHub #21）；另有“查看 Pi 扩展包更新…”与一行 Pi 扩展包状态（策略、本次检测到的包数、可更新的包、拒绝原因与上次执行失败告警，GitHub #22）；诊断窗口显示同一份只读状态与手动更新入口。
