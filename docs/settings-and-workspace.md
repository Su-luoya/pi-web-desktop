...
- 界面：菜单“服务 → 更新检查设置”里是状态行 + 四类快捷开关（打开 = 默认策略，关闭 = 关闭）+ “更新检查偏好设置…”；偏好设置窗口显示策略、每类最近检查时间、结果（最新 / 可更新 / 未知 / 失败）、被忽略版本与下次检查时间，并提供“忽略此版本”与启动前自动更新开关（GitHub #20，标题写明“仅限已验证的 npm 全局安装”）；诊断窗口显示同一份只读状态。
